...
describe('PresetsComponent', () => {
    ...
    it('A date should work', inject([FiltersStateService], (filterStateService: FiltersStateService) => {
        const now = new Date();

        component.values[2] = { $upperBound$: now };
        component.search(component.presets[2], 2);

        expect(filterStateService.get()).toBeTruthy();
        expect(filterStateService.get().filters).toBeTruthy();
        expect(filterStateService.get().filters.to.toLocaleDateString()).toBe(now.toLocaleDateString());
    }));
});
