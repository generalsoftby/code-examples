...
export class PresetsComponent implements OnInit {
    ...
    search(preset: Preset, index: number): void {
        ...
        forEach(value, (value: any, key: string): void => {
            if (preset.presetType === 'date') {
                value = formatDate(value, 'YYYY-M-D');
            }
     ...
     *
     * @param {BaseAutocompleteComponent} component
     * @param {number} index
     */
    ...
    change(preset: Preset, $event, index: number): void {
        ...
        const esQuery: Query = JSON.parse(preset.query),
     ...
     * @param {any}    field
     */
    selectValue(value: string, index: number, field: any): void {
