...
class NotificationService
{
    ...
    public function getNotificationRecipients(AbstractNotificationEvent $event, Notification $notification)
    {
        ...
        if (NotificationEventsEnum::MESSAGE_VIA_EMAIL_FROM_HELPDESK === $event->getEvent()) {
            if (NotificationChannelTypesEnum::EMAIL !== $notification->channel_type) {
                return collect();
            }
        ...
        if (NotificationEventsEnum::COMPANY_ADMIN_REGISTRATION_INVITATION == $event->getEvent()) {
            ...
            [
                'user' => $admin,
                'channel_type' => $notification->channel_type,
                'address' => NotificationChannelTypesEnum::EMAIL == $notification->channel_type ?
        ...
        $recipientTypes->each(function (NotificationRecipient $notificationRecipient) use (&$recipients, $event, $notification) {
            switch ($notificationRecipient->type) {
                case NotificationRecipientTypesEnum::BUYER_ORDER_RECIPIENT:
                    if (
                        NotificationRecipientUserTypesEnum::CUSTOMERS == $notification->recipient_user_type &&
                ...
                case NotificationRecipientTypesEnum::ADMIN_SHOWCASE_EVENT:
                    if (NotificationEventsEnum::REPLY_TO_TICKET === $event->getEvent()) {
                        /** @var null|HelpdeskComment $comment */
                        ...
                        if ($comment) {
                            ...
                            if (Admin::class == $comment->user_type) {
                    ...
                    } elseif ($showcase = $event->getShowcase()) {
                        /** @var Collection $allCompanyAdmins */
